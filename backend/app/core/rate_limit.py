...
class SlidingWindowRateLimiter:
    ...
    Kept in process memory: fine for a single backend container, but each replica would
    count separately and a restart resets the counters. A shared store (for example Redis)
    would be the next step if the app were scaled out.
    """
