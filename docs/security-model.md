...
- The client IP (rate limits, audit log) cannot be chosen by the client. `X-Forwarded-For`
  is read from the right: the entry added by a fixed number of trusted proxies
  (`TRUSTED_PROXY_HOPS`, compose), or the rightmost address outside trusted proxy networks
  (`TRUSTED_PROXIES`, Render). Before Phase 12 the leftmost entry was used, which let anyone
  bypass the per-IP limits; found by testing the live deployment, fixed from measured
  headers ([deployment.md](deployment.md)).
...
- Settings are validated at startup and by `python -m app.cli check-config`, which the
  container runs before anything else. SECRET_KEY must be at least 32 characters with at least
  10 distinct characters; DATABASE_URL must use the psycopg driver; with `APP_ENV=production`
  every CORS origin must be `https://` and `LOG_LEVEL=DEBUG` is refused.
- Validation errors name the setting, never its value (`hide_input_in_errors`), so a rejected
  secret does not end up in the logs. `check-config` does not print database driver errors
  either (they can contain the host and user).
- Containers: non-root users, `no-new-privileges`, all Linux capabilities dropped, read-only
  root filesystem (only `/tmp` is writable, in memory). nginx hides its version and sends CSP,
  X-Frame-Options, Permissions-Policy and Cross-Origin-Opener-Policy headers.

## Supply chain

- Backend dependencies are locked with exact versions and hashes (`requirements.txt`,
  `requirements-dev.txt`, generated by `uv pip compile --universal --generate-hashes`), and
  installed with `pip install --require-hashes`: a package whose download does not match the
  recorded hash is refused. The frontend installs from `package-lock.json` with `npm ci`.
- CI fails on known vulnerabilities (`pip-audit` on both lockfiles, `npm audit` at moderate and
  above) and on committed secrets (gitleaks over the full history; the allowlist names only the
  two fake AWS key IDs used by the redaction tests).
- Dependabot proposes weekly updates for pip, npm, GitHub Actions and the Docker base images;
  CI checks each one. Limitation: Dependabot edits the hashed lockfile directly; ranges in the
  `.in` files are updated by hand with `uv pip compile`.
- Not done: pinning GitHub Actions and base images to digests, container image scanning, SBOM
  generation.
