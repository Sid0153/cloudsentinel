...
2. nginx to backend: only the `X-Forwarded-For` entry nginx appends is trusted
   (`TRUSTED_PROXY_HOPS=1`); anything to its left came from the client. On Render the header
   is read from the right, skipping known proxy networks (`TRUSTED_PROXIES`, see
...
| Spoofing | Forged `X-Forwarded-For` to dodge rate limits or fake the audit IP | Header read from the right only: a fixed number of trusted proxies (compose) or the rightmost address outside trusted proxy networks (Render); checked through nginx in CI and against the live site | `core/client_ip.py`, `test_client_ip.py`, CI |
